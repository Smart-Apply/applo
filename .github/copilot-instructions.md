...
- **LLM (pluggable via `LLM_PROVIDER`):** `azure-openai` | `azure-ai-foundry` | `mistral` | `mock`
...
- `applications` — generation pipeline (profile + job → LLM → editor pass → keyword weave → style rewrite → length governor → grounding check → PDF → storage), SSE status stream. Split into two services: **`GenerationService`** (`generation.service.ts`) owns the create paths (`create`, `createWithGeneration`, `cancelPendingGeneration`, `generateWithSinglePipeline`) and every LLM pass (editor, keyword weave, style rewrites, length governor, grounding/style checks); **`ApplicationsService`** keeps CRUD, editor AI actions, export/regenerate, files, SSE, and keyword analysis (edit-mode cover-letter regeneration delegates to `GenerationService` for `detectLanguage`/`extractJobFacts`/`runLengthGovernorPass`; the shared response mapper + Markdown→HTML conversion live in `application-response.util.ts` / `cover-letter-html.util.ts`). Owns the `grounding/` sub-service (`GroundingValidatorService`) — a deterministic, non-destructive anti-hallucination check that flags impact numbers (%, currency, counts) absent from the source profile (logs only; never strips), plus a deterministic **style check** (`style-lint.util.ts`) that flags forbidden AI clichés + German Konjunktiv/hedging on the finished documents (also logs-only, never strips), and a **guarded style-rewrite pass** (`prompts/v1/style-rewrite.md` + `evaluateStyleRewrite`) that surgically fixes those flagged clichés/hedging in the cover letter — non-destructive (ships only a strictly-cleaner, non-gutted rewrite; never fabricates; graceful fallback to the pre-rewrite draft). A matching **résumé style-rewrite** (`prompts/v1/resume-style-rewrite.md` + `evaluateResumeStyleRewrite`) does the same JSON→JSON over the résumé prose (summary/achievements/highlights), ID-preserving via `isValidResumeEdit` (only ships a valid, strictly-cleaner payload). Its résumé violation count also flags German **verb-first bullets** (the anglicised "Entwickelte…" opener via `detectGermanVerbFirstBullets`), which the résumé teeth convert to Nominalstil. Cover-letter **length is deterministically governed** the same way: `lintCoverLetterLength` (in `style-lint.util.ts`) measures body words against the persisted budget (`Application.coverLetterLength` `kurz`/`standard` → `COVER_LETTER_BUDGETS` in `constants.ts`; `{{lengthBudget}}` is rendered into the cover-letter/editor/weave prompts), a guarded `prompts/v1/shorten-cover-letter.md` pass (`runLengthGovernorPass` + `evaluateShortenRewrite`) fires only on a measured overrun and only ships a within-budget, non-gutted, salutation-preserving, keyword-retaining cut (graceful fallback), and the export processor adds a `pdf-parse` **page-count backstop** that warns when a rendered cover letter exceeds one page. Also owns the coverage-driven keyword loop (#6): `keyword-coverage.util.ts` selects the priority-1, **profile-supported** ATS keywords still missing from the cover letter and a single guarded `prompts/v1/keyword-weave.md` pass weaves them into the existing prose (never invents unsupported keywords; graceful fallback to the pre-weave draft). Edit-mode cover-letter regeneration (`upsertCoverLetter`) reuses the v1 `cover-letter.md` prompt via `stored-resume.util.ts` (`mapStoredResumeToTailoredProfile`) instead of a separate ATS prompt (#2). Also owns **translation-on-export** (`translation/` sub-module — `TranslationModule` + `TranslationService`, consumed by the export job processor in `jobs/`): cross-language exports (targets `de`/`en`/`fr`/`es`/`pt`/`it`; source content is always de/en) translate the stored content via segment-based strict-`json_schema` résumé translation (`translation-segments.util.ts` + `prompts/v1/translate-resume.md`) and HTML→HTML cover-letter translation (`prompts/v1/translate-cover-letter.md`), guarded all-or-nothing with fallback to a fully source-language PDF + `exportWarning: 'TRANSLATION_FALLBACK'`; successes are cached per language in `Application.translations` keyed by content xxhash (`utils/translation.util.ts`). Date labels are re-derived deterministically per language from raw ISO dates by `resume-date-localizer.util.ts` (token-mapping for legacy rows) — never via LLM.
...
7. **Style rewrite + length governor + grounding/style checks (#3/#7):** A guarded `prompts/v1/style-rewrite.md` pass first surgically fixes the AI clichés + German Konjunktiv/hedging the deterministic linter flags in the cover letter — it only ships a rewrite that the `evaluateStyleRewrite` guard confirms is both non-gutted and strictly cleaner, never fabricates, and falls back to the pre-rewrite draft otherwise. A matching `prompts/v1/resume-style-rewrite.md` pass (run right after the résumé editor) does the same JSON→JSON over the résumé prose, guarded by `evaluateResumeStyleRewrite` (ID-preserving via `isValidResumeEdit` + strictly cleaner); its violation count combines clichés/hedging with German **verb-first résumé bullets** (`detectGermanVerbFirstBullets`, Nominalstil enforcement). Next the **length governor**: `lintCoverLetterLength` (in `style-lint.util.ts`) deterministically counts the letter's body words (salutation/closing excluded) against the application's word budget (`COVER_LETTER_BUDGETS` in `applications/constants.ts` — `kurz` 250 / `standard` 350, DTO-selectable, persisted on `Application.coverLetterLength`; the prompts receive **both** `{{lengthBudget}}` and `{{lengthTargetMin}}` (= `COVER_LETTER_TARGET_MIN_FACTOR` 0.85 × budget → 298 / 213) and ask for that **band**, not a bare ceiling — a "max N words" instruction alone has nothing pushing back on a short letter, which measured 70 % of budget on gpt-4.1 and 61 % on gpt-5.4-mini); on a real overrun a single guarded `prompts/v1/shorten-cover-letter.md` pass cuts redundancy/filler, accepted only when `evaluateShortenRewrite` confirms it lands within budget, isn't gutted, keeps the salutation verbatim, doesn't regress style violations, and retains every woven priority-1 keyword (graceful fallback to the pre-shorten draft; never mechanical truncation). Then `GroundingValidatorService` flags fabricated impact numbers vs. the profile (non-destructive, logs a warning only), and a deterministic **style + length check** (`style-lint.util.ts`) flags any residual forbidden AI clichés + German Konjunktiv/hedging and word-budget overruns on the finished documents (non-destructive, logs only). The two LLM generation calls (cover letter, resume rewrite) also carry a shared `GENERATION_SYSTEM_ANCHOR` system message — the recommended system/user split that keeps the non-negotiable constraints in the system turn.
...
## Prompt Templates
The active generation prompts live under `apps/api/prompts/v1/*` and are described in the **Application Pipeline** above: `skill-selector`, `job-facts`, `cover-letter`, `resume`, `resume-rewrite`, `ats-keywords`, `editor-cover-letter`, `editor-resume`, `keyword-weave`, `style-rewrite`, `resume-style-rewrite`, `shorten-cover-letter` (the guarded length-governor pass; the cover-letter/editor/weave prompts all consume a `{{lengthBudget}}` variable resolved from `Application.coverLetterLength`). A separate on-demand `application-validation` prompt (strict `json_schema`) powers `POST /validation` (the standalone Bewerbungs-Check of an application the user created outside Applo; not part of the create pipeline). Two on-demand translation prompts power cross-language export (`POST /applications/:id/export` with a different `language`): `translate-resume` (strict `json_schema`, flat `{id, text}` segments — ids/dates/URLs never pass through the LLM) and `translate-cover-letter` (HTML→HTML); both are guarded with fallback to the source language. The legacy top-level `cover-letter.md`, `resume.md`, `cover-letter-ats.md` and `resume-ats.md` prompts were **retired (#2)**. The editor's "regenerate cover letter" action now reuses `v1/cover-letter.md` via `stored-resume.util.ts` (`mapStoredResumeToTailoredProfile` maps the saved editor resume back into the `TailoredProfileDto` the v1 prompt expects), so there is a single cover-letter generation path.
