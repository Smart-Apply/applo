...
7. **Grounding check (#7):** `GroundingValidatorService` flags fabricated impact numbers vs. the profile (non-destructive, logs a warning only). A deterministic **style check** (`style-lint.util.ts`) then flags forbidden AI clichés + German Konjunktiv/hedging on the finished documents (non-destructive, logs only). The two LLM generation calls (cover letter, resume rewrite) also carry a shared `GENERATION_SYSTEM_ANCHOR` system message — the recommended system/user split that keeps the non-negotiable constraints in the system turn.
