...
7. **Style rewrite + grounding/style checks (#3/#7):** A guarded `prompts/v1/style-rewrite.md` pass first surgically fixes the AI clichés + German Konjunktiv/hedging the deterministic linter flags in the cover letter — it only ships a rewrite that the `evaluateStyleRewrite` guard confirms is both non-gutted and strictly cleaner, never fabricates, and falls back to the pre-rewrite draft otherwise. Then `GroundingValidatorService` flags fabricated impact numbers vs. the profile (non-destructive, logs a warning only), and a deterministic **style check** (`style-lint.util.ts`) flags any residual forbidden AI clichés + German Konjunktiv/hedging on the finished documents (non-destructive, logs only). The two LLM generation calls (cover letter, resume rewrite) also carry a shared `GENERATION_SYSTEM_ANCHOR` system message — the recommended system/user split that keeps the non-negotiable constraints in the system turn.
