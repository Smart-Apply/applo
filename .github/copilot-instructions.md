...
**Applo is designed to work across ALL professions and industries**, not just IT/tech roles. When writing code, documentation, or examples:
...
- **LLM (pluggable via `LLM_PROVIDER`):** `azure-openai` | `azure-ai-foundry` | `mistral` | `mock` | `fake`
...
- `admin` — allow-listed admin endpoints (gated by `ADMIN_EMAILS` env), e.g. `POST /admin/users/:email/tier`, `DELETE /admin/users/:email`. Also owns `llm-usage/` — read-only, aggregate-only analytics over `LlmUsageEvent` (`GET /admin/llm-usage/{summary,breakdown,timeseries}`, issue #525); no `actorHash` filter, group key or field ever leaves the module.
- `applications` — generation pipeline (profile + job → LLM → editor pass → keyword weave → style rewrite → length governor → grounding repair/check → PDF → storage), SSE status stream. Split into two services: **`GenerationService`** (`generation.service.ts`) owns the create paths (`create`, `createWithGeneration`, `cancelPendingGeneration`, `generateWithSinglePipeline`) and every LLM pass (editor, keyword weave, style rewrites, length governor, grounding repairs/checks); **`ApplicationsService`** keeps CRUD, editor AI actions, export/regenerate, files, SSE, and keyword analysis (edit-mode cover-letter regeneration delegates to `GenerationService` for `detectLanguage`/`extractJobFacts`/`runLengthGovernorPass`; the shared response mapper + Markdown→HTML conversion live in `application-response.util.ts` / `cover-letter-html.util.ts`). Owns the `grounding/` sub-service (`GroundingValidatorService`) — a deterministic anti-hallucination check for unsupported impact numbers (résumé corpus = profile only; cover-letter corpus = profile + job posting), plus guarded repair passes: `fix-unsupported-numbers.md` repairs cover-letter prose, while `fix-unsupported-numbers-resume.md` repairs only rewritten résumé prose via strict JSON. `evaluateResumeGroundingRepair` requires `isValidResumeEdit`, strictly fewer unsupported values, no replacement fabrication, and no style regression; any rejection keeps the pre-repair payload. Neither `createWithGeneration` nor `generateWithSinglePipeline` implements the chain any more — both delegate to `headless/generate.ts` (see below) and keep only ownership checks, SSE progress, the grounding/style reports and persistence. The module also owns a deterministic **style check** (`style-lint.util.ts`) and guarded style-rewrite passes for the cover letter (`prompts/v1/style-rewrite.md`) and résumé (`prompts/v1/resume-style-rewrite.md`, ID-preserving; also converts flagged German verb-first bullets to Nominalstil). Cover-letter **length is deterministically governed** by `lintCoverLetterLength` plus guarded `prompts/v1/shorten-cover-letter.md`; the export processor adds a `pdf-parse` page-count backstop. The coverage-driven keyword loop (`keyword-coverage.util.ts` + `prompts/v1/keyword-weave.md`) adds only priority-1 profile-supported gaps. Edit-mode cover-letter regeneration (`upsertCoverLetter`) reuses `v1/cover-letter.md` via `stored-resume.util.ts`. The module also owns **translation-on-export** (`translation/`): strict segment-based résumé translation + guarded HTML cover-letter translation to de/en/fr/es/pt/it, cached by source-content hash; date labels are re-derived deterministically, never translated by the LLM.
...
- `llm` — pluggable providers (`azure-openai` | `azure-ai-foundry` | `mistral` | `mock` | `fake`) with automatic language detection, opossum circuit breaker, Azure **structured outputs** (#8: strict `json_schema` for `ats-keywords`/`resume-rewrite`, `json_object` JSON mode elsewhere, resolved by template path in `schemas/v1-schemas.ts`; regex JSON repair kept as a fallback), and **three routing lanes**, each with its own independent circuit breaker and its own fallback to the main lane (`callRouted`; `resolveRouting` precedence = explicit `model` → mid → fast → main):
  - **main** — `LLM_PROVIDER` + `AZURE_OPENAI_*` (`gpt-4.1`). Candidate-facing prose. Always the floor: a side-lane failure re-dispatches here *without* the model override.
  - **fast** — `LLM_FAST_MODEL` (+ optional `LLM_FAST_PROVIDER` when it lives on a different provider; today `mistral-small-latest` on Mistral). Per-**task** routing for the mechanical steps listed in `FAST_MODEL_TEMPLATES` (`ats-keywords`, `job-facts`, `skill-selector`, `interview-*`). No-op when unset.
  ...
  ⚠️ **Reasoning models need a different request shape.** The GPT-5 family (incl. `gpt-5.4-mini`) and the o-series reject `temperature`, `top_p`, penalties, logprobs **and `max_tokens`** — the output cap is `max_completion_tokens` instead. `providers/model-tuning.util.ts` (`isReasoningModel` + `buildModelTuningParams`) shapes the body per family; sending the classic parameters returns `400 Unsupported parameter`. It also sends `reasoning_effort` (`AZURE_OPENAI_REASONING_EFFORT`, default `minimal`, overridable per call) and adds **headroom** to `max_completion_tokens`, because hidden reasoning tokens draw from the same budget as the visible answer — without it a reasoning model returns empty content with `finish_reason: 'length'`. Two consequences when pointing a lane at one: the pipeline's tuned per-call temperatures (0.2 skill-selector, 0.35 resume-rewrite, 0.3 revision passes) are **silently ignored**, and hidden reasoning tokens are **billed as output** — so a reasoning model's real output cost can far exceed its visible completion length.

  **Offline/eval provider (`LLM_PROVIDER=fake`).** `providers/fake-v1.provider.ts` is a deterministic, v1-chain-aware provider: unlike `mock` it satisfies the `callJson` steps, so the FULL chain runs with no network and no cost. It detects the pipeline step from the prompt's `# Role:` heading — **not** from a fixed-size window at the top, which prompt caching's STABLE PREFIX silently broke once (every step misdetected, every call returned `{}`, exit code 0). Selecting `fake` also forces the fast and mid lanes off (`createFastProvider`/`createMidProvider` return `null`); without that, `LLM_FAST_PROVIDER=mistral` kept billing real calls on a run labelled offline and destroyed run-to-run determinism.

  **Per-call telemetry.** `callText`/`callJson` accept an optional `onCallMeta` hook reporting the same resolved values the usage recorder gets (serving model, lane, latency, tokens) on both the success and failure path. Used by the headless seam; there is deliberately no second telemetry mechanism.

  **Usage tracking (issue #522, `llm/usage/`):** every `callText`/`callJson`/`generateText`/`translateSummary` call — regardless of lane, provider, or success/failure — records one `LlmUsageEvent` row (`LlmUsageService.record`, fire-and-forget, never throws). Rows record the lane/model that **actually served** the call, so a side-lane fallback is attributed to `main`, not to the lane that failed. `feature` is derived from the exact template path (`usage/llm-feature.map.ts`, exact-key lookup — deliberately NOT `includes()`, unlike `FAST_MODEL_TEMPLATES` above). `userId` reaches the recorder only via an `AsyncLocalStorage` actor scope — `LlmUsageContextInterceptor` (global `APP_INTERCEPTOR`) seeds it from the JWT-authenticated request for every HTTP call; the two non-HTTP paths seed it explicitly (`ApplicationProcessor.process` for queued generation jobs, `MailboxSyncOrchestrator` for the public Graph webhook). No-op entirely when `LLM_USAGE_HASH_SALT` is unset. The only read path is the aggregate-only admin analytics in `admin/llm-usage/` (issue #525) — raw rows are never exposed.
  ...
  **Anonymised dataset export (issue #523, `usage/llm-usage-export.service.ts`):** `LlmUsageExportService` turns those pseudonymous rows into a portable ML/due-diligence artefact, served by two admin-only endpoints (`GET /admin/llm-usage/export`, `.../export/manifest`). It is the *export* that anonymises — the live table stays as described above. Transformations, in order: explicit source-column allow-list (a new column can't leak in by accident) → row `id` dropped (it's a cuid embedding a millisecond timestamp, so exporting it would undo bucketing; it is read for cursor paging only) → `createdAt` truncated to a UTC `hour|day|month` bucket (**no raw-timestamp option exists**) → `latencyMs` rounded to 10 ms → the stored `actorHash` re-keyed with a random per-export salt that is discarded (`actor=pseudonym`, unlinkable across exports and to the live table) or omitted (`actor=none`) → **k-anonymity** over `timeBucket/feature/tier/language/model`, counting **distinct actors, not rows** (one heavy user's k calls must not pass as a crowd; actor-less rows carry no personal data and are always released) → total ordering by exported column values only, so read order and sub-bucket call sequence aren't observable and `actor=none` output is byte-reproducible. Serializes to JSONL or CSV (formula-injection neutralised); **Parquet is deliberately a downstream conversion** (DuckDB one-liner in the doc), not a backend dependency. Every export ships with a manifest stating parameters, counts and guarantees — the file and its manifest travel together. Full schema + anonymity guarantees + the two sanctioned parameter profiles: [docs/security/LLM_USAGE_DATASET.md](../docs/security/LLM_USAGE_DATASET.md). Bump `LLM_USAGE_EXPORT_SCHEMA_VERSION` and update that doc whenever the column set changes.
...
- Also present, not detailed above: **UserPreferences** (`analyticsEnabled` is a real opt-out — read by `LlmUsageService.record()` at the write path; `onboardingCompleted` is the first-login product-tour flag; `profilePublic` is RETIRED per issue #806 and kept as a column only until a follow-up release drops it), **SubscriptionUsage**, **BackgroundJob**, **TwoFactorAuth**, **TwoFactorBackupCode**, **TrustedDevice**, **OAuthProvider**, and the interview trio **InterviewSession**/**InterviewQuestion**/**InterviewFeedback** (the "Interview" shorthand above), plus **Template** (the "ResumeTemplate" shorthand)
...
**GET /api/v1/admin/llm-usage/export** — streamed anonymised `llm_usage_events` dataset (issue #523)
  - Query: `format` (`jsonl` default | `csv`), `from`/`to` (ISO 8601, `from` inclusive / `to` exclusive), `bucket` (`hour` default | `day` | `month`), `actor` (`pseudonym` default | `none`), `k` (1–1000, default 5), `limit` (1–200000, default 50000)
  - Never emits the row `id`, the raw `createdAt` or the stored `actorHash`; response headers carry `X-Applo-Export-Schema-Version`, `X-Applo-Export-Rows`, `X-Applo-Export-Suppressed-Rows`
  - `k=1` disables suppression (internal debugging only); `actor=none&bucket=day&k=5` is the profile for anything that leaves the company
**GET /api/v1/admin/llm-usage/export/manifest** — same query params; returns the manifest (parameters, row/suppression counts, column schema, guarantees, residual risks) for the export those params produce. Ship it alongside the data file.
  - Schema, transformations and anonymity guarantees: [docs/security/LLM_USAGE_DATASET.md](../docs/security/LLM_USAGE_DATASET.md)

**LLM token-usage analytics** (issue #525) — read-only aggregation over `LlmUsageEvent`, same `ADMIN_EMAILS` gate. Every response is an **aggregate**: `actorHash` is never a filter, a group key or part of a payload (only `count(DISTINCT "actorHash")` as `distinctActors`), so nothing here resolves back to a `User`. Shared query params on all three: `from`/`to` (ISO-8601 half-open window, default = last 30 days) plus optional `feature`/`tier`/`provider`/`lane`/`language` filters. Every metric block carries `calls`, `successes`, `failures`, `successRate`, prompt/completion/cached/total tokens, `estimatedCostUsd`, `avgLatencyMs`, and the coverage counts `callsWithUsage`/`callsWithCost` — the cost sum is a **lower bound**, since unpriced models (e.g. `gpt-5.4-mini`) and calls without reported usage contribute 0.

**GET /api/v1/admin/llm-usage/summary** — totals for the window
**GET /api/v1/admin/llm-usage/breakdown** — totals grouped by one dimension
  - `groupBy` (required): `feature | tier | language | model | provider | lane`; `limit` 1–200 (default 50), ordered cost → tokens → calls
**GET /api/v1/admin/llm-usage/timeseries** — totals per time bucket
  - `interval`: `day | week | month` (default `day`, UTC); optional `groupBy` for e.g. cost per feature per day; `truncated: true` when the series hits the 2000-point cap

Implementation note: aggregation uses composed `Prisma.sql` (Prisma's typed `groupBy` can't express `date_trunc`). Caller values are **always** bound parameters and group-by identifiers come from a `switch` over the allow-listed union — never `Prisma.raw`/`$queryRawUnsafe` on request input.
...
### Appointments — Interview calendar (Protected)

User-scoped calendar entries surfaced on the dashboard calendar card. All routes require JWT; every query is scoped by `userId`.

**GET /api/v1/appointments** — list the user's appointments, chronological (`date`, then `startTime`)
**POST /api/v1/appointments** — create (`date` "YYYY-MM-DD" required, optional `startTime` "HH:mm", `note` required, `emailReminder` default false)
**PATCH /api/v1/appointments/:id** — partial update (404 if not owned)
**DELETE /api/v1/appointments/:id** — delete (204; 404 if not owned)
...
- Body: `{ jobPostingId, notes?, coverLetterLength? ('kurz' | 'standard', default 'standard') }`
...
- Body: `{ language?: 'de' | 'en' | 'fr' | 'es' | 'pt' | 'it' }` — generation stays de/en (job-posting driven); fr/es/pt/it are export-time translation targets
- Cross-language exports translate the stored content on the fly (guarded segment-based résumé translation + HTML cover-letter translation, cached in `Application.translations` by content hash); date labels are re-derived deterministically per language
- On translation failure the PDFs render fully in the source language and the response carries `exportWarning: 'TRANSLATION_FALLBACK'` (surfaced as a toast)
- Persists the requested language to `Application.language`; returns the updated application (status GENERATING, completion via SSE)

### Validation — Bewerbungs-Check (Protected)

Standalone AI quality + ATS check of an application the user created **outside** Applo. Not tied to a generated `Application`/`JobPosting`.
...
- **Dedupe:** an exact re-submit (same normalized résumé + cover letter + jobContext + language, matched on `Validation.contentHash` scoped to `userId`) returns the stored record from the controller before `reserveUsage` runs — **no LLM call, no credit spent**. `title` is excluded from the hash, so relabelling still hits. Note: `UsageLimitGuard` still runs first, so a user already at their limit gets the 403 before the cache is consulted; they can still reach the stored result via `GET /validation`.
...
7. **Style rewrite + length governor + grounding repair/checks (#3/#7):** Guarded `style-rewrite.md` and `resume-style-rewrite.md` passes fix only deterministic style findings; résumé edits must preserve every source ID. The cover-letter length governor runs next and accepts only a within-budget, non-gutted, salutation-preserving, keyword-retaining result. Two final grounding passes then repair unsupported impact figures: `fix-unsupported-numbers.md` uses profile + job-posting evidence for the cover letter, while strict-JSON `fix-unsupported-numbers-resume.md` uses the profile only and runs after résumé style rewrite. `evaluateResumeGroundingRepair` rejects malformed/ID-mangled payloads, non-improvements, replacement fabrications, and style regressions. The final `GroundingValidatorService` + style/length checks report any residual issue without destructively altering content. The generation calls and all guarded revisions carry `GENERATION_SYSTEM_ANCHOR`.
8. TSX → PDF via `@react-pdf/renderer` (no browser, no post-processing). The export processor also runs a deterministic **page-count backstop** (`pdf-parse`) that logs a warning when a rendered cover letter exceeds one page — the visual "2 Seiten" failure word counts can miss.
9. Upload PDFs via storage provider (Blob/S3/disk); persist keys + signed URLs
10. Status: `PENDING → GENERATING → READY | FAILED` (pushed to client via **SSE**; pipeline progress is persisted on the row — `Application.generationProgress`/`generationMessage` — so the SSE poll serves it correctly from either prod machine)
...
## Prompt Templates
The active generation prompts live under `apps/api/prompts/v1/*` and are described in the **Application Pipeline** above: `skill-selector`, `job-facts`, `cover-letter`, `resume-rewrite`, `ats-keywords`, `editor-cover-letter`, `editor-resume`, `keyword-weave`, `style-rewrite`, `resume-style-rewrite`, `shorten-cover-letter`, `fix-unsupported-numbers`, and `fix-unsupported-numbers-resume`. The résumé grounding repair shares the strict `resume_rewrite` JSON schema with the writer/editor/style passes. A separate on-demand `application-validation` prompt powers `POST /validation`. Two guarded translation prompts power cross-language export: strict segment-based `translate-resume` and HTML `translate-cover-letter`. The legacy top-level `cover-letter.md`, `resume.md`, `cover-letter-ats.md` and `resume-ats.md` prompts were retired. The editor's cover-letter regeneration reuses `v1/cover-letter.md` via `stored-resume.util.ts`.
...
# Mid lane: opt-in per call via { midLane: true } (today: Free-tier
# Bewerbungs-Checks). Endpoint/key fall back to AZURE_OPENAI_* when unset, so a
# mid model in the MAIN resource only needs LLM_MID_MODEL. The endpoint must be
# the RESOURCE ROOT — a project-scoped Foundry URL (.../api/projects/<name>) is
# the Agents SDK form and 404s here. Unset = lane off.
# LLM_MID_MODEL=gpt-5.4-mini
# AZURE_OPENAI_MID_ENDPOINT=https://foundry-applo-prod.services.ai.azure.com
# AZURE_OPENAI_MID_API_KEY=<key>
...
# Reasoning effort for GPT-5-family/o-series deployments (minimal | low | medium
# | high); ignored for classic models. Also drives the max_completion_tokens
# headroom — see the reasoning-model warning in the `llm` module notes.
AZURE_OPENAI_REASONING_EFFORT=minimal

# Mistral (opt-in, A/B) — OpenAI-wire-compatible; La Plateforme OR Azure AI Foundry.
# See docs/guides/LLM_MODEL_SELECTION.md; gate on a json_schema eval before switching.
# La Plateforme: MISTRAL_ENDPOINT=https://api.mistral.ai/v1, MISTRAL_MODEL=mistral-large-latest, NO api-version.
# Azure Foundry: MISTRAL_ENDPOINT=https://<res>.services.ai.azure.com/models, MISTRAL_MODEL=Mistral-Large-3, set MISTRAL_API_VERSION.
MISTRAL_ENDPOINT=https://api.mistral.ai/v1
MISTRAL_API_KEY=<key>
MISTRAL_MODEL=mistral-large-latest
# MISTRAL_API_VERSION=2024-05-01-preview   # set ONLY for Azure AI Foundry
...
**Migrations** run as a Fly **release command** (`prisma migrate deploy`) before machines start serving traffic — same command on staging and prod, against each env's `DIRECT_URL` secret. `flyctl deploy --image` still runs the release command, so build-once-promote doesn't change migration behavior.

**Template catalog** is synced by the same release command, chained after the migration: `node apps/api/prisma/dist/prisma/seed-react-pdf-templates.js`. Template rows live in the DB but are **code-owned** (every active row must resolve to a factory in `pdf-v2/template-registry.ts`), so a new TSX design is invisible in staging/prod until the seed runs. The seed is idempotent (upserts by id, deactivates — never deletes — unrenderable rows) and a failure aborts the deploy. Adding a design therefore means: register the factory **and** add it to the seed's `DESIGNS` array — nothing else is needed to publish it.

**Build-once-promote (API):** the Docker image is built a single time per commit on the GHA runner (GitHub-native `type=gha` layer cache — no Turbo Remote, no Fly remote builder) and pushed to `registry.fly.io/smart-apply-api:sha-<gitsha>`. Staging deploys it on every api-touching push; prod promotes the **same SHA image** on tag. This removes the old redundant cold prod rebuild (~11 min → ~2–3 min). Requires the **`FLY_REGISTRY_TOKEN`** repo secret — an org-scoped Fly token (`flyctl tokens create org personal`) that can both push to `registry.fly.io` and deploy both apps (staging pulls the shared prod-namespace image). The single-arch image sets `provenance: false` so `flyctl deploy --image` receives a runnable manifest.
