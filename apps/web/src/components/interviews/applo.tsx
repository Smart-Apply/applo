...
/* ============================================================
   Applo — flat 2D mascot for the Interview Coach.
   Rigged with separate layers so each part animates independently
   (antennas · eyes/screen · arms · body). State is driven by a
   class on the <svg> (state-idle / state-wave / state-think /
   state-process / state-success / state-done) handled in
   applo-mascot.css. Flat style only — no gradients/shadows/glow.
   ============================================================ */

import './applo-mascot.css';

const NAVY = '#1B2A49'; // matches --primary
const SCREEN = '#EEF3FB';
const BORDER = 'rgba(27,42,73,.10)';
const BLUE = '#3B82F6'; // matches --accent
const GREEN = '#16A34A'; // matches --success
const GEARBG = '#E7EEFA';

export type ApploState = 'idle' | 'wave' | 'think' | 'process' | 'success' | 'done';
...
function Gear({ cx, cy, r, teeth = 8, fill, cls }: GearProps) {
  const tw = r * 0.42;
  const th = r * 0.6;
  const ts = [];
...
export function Applo({ state = 'idle', size = 240, className, ...rest }: ApploProps) {
  const S = NAVY;
  return (
    <svg
    ...
    >
      <g className="root-float">
        <g className="rig">
          ...
          {/* ---- ARM: right hand to chin (thinking), in front of face ---- */}
          ...
          {/* ---- FX: processing — gears in the belly ---- */}
          ...
          {/* ---- FX: thinking — thought bubble w/ dots ---- */}
          <g className="fx-dots">
