...
export class AtsValidatorService {
  ...
  async getDetailedReport(pdfBuffer: Buffer): Promise<AtsValidationReport> {
    try {
      ...
      const selectableTextCheck = textBasedCheck; // Same check
   ...
   * Note: pdf-lib doesn't provide direct font inspection, so we use heuristics
   */
  private async checkFonts(pdfDoc: PDFDocument): Promise<boolean> {
    try {
      // For MVP, we assume fonts are safe if the PDF was generated by our system
      // More sophisticated font detection would require parsing the PDF content stream
      // which is beyond the scope of this implementation
      
      // In practice, if we control PDF generation (which we do), we know we're using
      // Arial/Helvetica which are ATS-safe
      return true;
    } catch (error) {
