...
export class AtsValidatorService {
  ...
  private async checkFonts(pdfDoc: PDFDocument): Promise<boolean> {
    try {
      ...
      // If no embedded fonts, assume standard fonts are used (good for ATS)
      if (embeddedFonts.length === 0) {
        return true;
      }

      // Check if embedded fonts are ATS-safe
      for (const font of embeddedFonts) {
        const fontName = font.name.toLowerCase();
        const isSafe = this.ATS_SAFE_FONTS.some(safeFont => fontName.includes(safeFont));
        if (!isSafe) {
          return false;
        }
      }
  ...
  private async checkFontsDetailed(pdfDoc: PDFDocument) {
    try {
      const embeddedFonts = pdfDoc.getEmbeddedFonts();
      const fontsDetected = embeddedFonts.map(f => f.name);
      const passed = await this.checkFonts(pdfDoc);
      
      return {
        passed,
        message: passed
          ? 'PDF uses ATS-safe fonts'
  ...
  private async checkMetadataDetailed(pdfDoc: PDFDocument) {
    try {
      const title = pdfDoc.getTitle();
      const author = pdfDoc.getAuthor();
      const keywords = pdfDoc.getKeywords() ? pdfDoc.getKeywords().split(',').map(k => k.trim()) : [];
