...
- **ATS-optimized PDFs** — 50 templates (5 designs × 5 languages × 2 types) rendered via `@react-pdf/renderer` (TSX). Template previews via `pdfjs-dist` + `@napi-rs/canvas`. In edit mode, list items (jobs, projects, education, …) and whole sections can be reordered; the exported PDF honors the chosen order (optional `sectionOrder` in the résumé JSON — absent records keep the template default).
