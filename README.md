...
- **AI generation** — Azure OpenAI with pluggable providers (`azure-openai` | `azure-ai-foundry` | `mock`), wrapped in an opossum circuit breaker; JSON calls use Azure structured outputs (strict `json_schema` / JSON mode) so responses are schema-valid by construction; a self-review editor pass refines the cover letter, a coverage-driven keyword loop weaves in missing profile-supported ATS keywords, a deterministic grounding check flags fabricated metrics, a deterministic style check flags AI clichés and German hedging, and a guarded style-rewrite pass then surgically fixes those flagged clichés/hedging — and anglicised German verb-first bullet openers — in the cover letter and résumé (non-destructive)
