...
- **ATS-optimized PDFs** — 3 designs × 3 Farbvarianten × 2 Dokumenttypen rendered via `@react-pdf/renderer` (TSX; language-neutral components, labels follow the document language). Per-application design tuning via `templateSettings`: font scale (±8 %), vertical density, a free accent-color override, and curated font families (bundled OFL fonts: Lato, Source Sans 3, Merriweather) — adjustable in the editor's "Design" panel (live approximation in the edit view; the exported PDF is authoritative). Template previews via `pdfjs-dist` + `@napi-rs/canvas`. In edit mode, list items (jobs, projects, education, …) and whole sections can be reordered; the exported PDF honors the chosen order (optional `sectionOrder` in the résumé JSON — absent records keep the template default).
