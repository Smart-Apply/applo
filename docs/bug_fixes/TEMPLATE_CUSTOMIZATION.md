...
> **Status:** In progress — shipped so far: §3.1 catalog fix (✅ 2026-07-22), §3.2+§3.3 per-application design settings (✅ 2026-07-22), §3.4 font bundling (✅ 2026-07-22), §3.6 design panel (✅ 2026-07-22), §3.5 photo support (✅ 2026-07-22: `Profile.photoKey` storage key + `POST/GET/DELETE /profile/photo` (JPEG/PNG ≤ 2 MB, magic-byte-validated), profile-page upload avatar, `templateSettings.showPhoto` toggle in the Design panel with ATS-Hinweis + profile link, export processor resolves the photo to a data URI, all 3 templates render a 3:4 portrait `<Image>` (header top-right / sidebar top), editor mimic shows it live, storage object deleted best-effort with the account. Deltas: field named `photoKey` (it stores a storage key, not a URL); crop-to-portrait skipped — templates center-crop via `objectFit: cover`). Next: §3.7 new layouts. · **Priority:** P2 (competitor-parity; contained one real P1 bug in §2.5 — **✅ fixed 2026-07-22**, see §3.1)
