...
| Phase | Scope | Status |
|---|---|---|
| **1** | Eval statistical power | ✅ Complete |
| **1a** | Pooled claim-rate metric + CI (free) | ✅ Complete |
| **1b** | `--repeat=N` + aggregation across runs | ✅ Complete |
| **1c** | Paired A/B comparison tool | ✅ Complete |
| **2** | Résumé grounding repair | ✅ Complete |
| **2a** | Prompt + guard | ✅ Complete |
| **2b** | Pipeline + harness wiring | ✅ Complete |

---

## Phase 1 — Eval statistical resolution

**Goal:** quantify fabrication magnitude beyond binary fixture pass/fail and report
stochastic uncertainty without overstating the independent sample size.
...
| Run | fixture pass-rate (n=24) | pooled values | unsupported | value rate | descriptive SE |
|---|---|---|---|---|---|
...
This value-level view distinguishes one unsupported number from several within a failed
fixture at no additional token cost. Because normalized values are de-duplicated and clustered
inside fixtures, the SE column is historical/descriptive, not a power calculation.
...
- `aggregate.ts`: add `grounding.totalClaims`, `grounding.unsupportedClaims`,
  ...
  actively wrong). The validator de-duplicates normalized values per fixture, so treat
  the interval as descriptive rather than as an independence-based power claim; when
  no values are checked, persist `null` and print `n/a`.
...
**Acceptance criteria**
- [x] Claim rate + Wilson CI printed and persisted in the results JSON.
- [x] Recomputed for the eight 2026-08-11 runs and recorded in the Changelog.
- [x] Fixture pass-rate retained (do not break comparability with existing result files).

### 1b. `--repeat=N` `[ Cheap — linear cost ]`

LLM output is stochastic; one run per arm confounds the change under test with sampling
noise. Repeat each arm to expose within-fixture variation and estimate each fixture's mean
more reliably. Repeats do **not** create new independent fixtures: `k=3` produces 72 generated
observations, but inference still uses 24 fixture clusters. More repeats improve stability
measurement; more profession-diverse fixtures are required to increase the independent sample
size. Default `k=1`; use `k≥3` when stochastic stability matters.
...
- `compare.ts`: match generated observations by fixture + repeat, then cluster repeats by
  fixture before inferential tests.

**Acceptance criteria**
- [x] `--repeat=3` produces one pooled summary plus per-repeat figures.
- [x] The observed spread across repeats is reported, not hidden.

### 1c. Paired comparison tool `[ Controls fixture variance ]`

Comparing two independent runs discards the fact that both arms ran the **same fixtures**.
A paired comparison controls fixture-to-fixture variance by comparing per-fixture outcomes;
repeated observations are averaged within each fixture before inference.
...
**Acceptance criteria**
- [x] `pnpm eval:compare <a.json> <b.json>` prints per-fixture deltas + a significance verdict.
- [x] Re-run over the 2026-08-11 pairs; confirm exact McNemar remains non-significant and
  continuous metrics report fixture-clustered intervals.
...
| `scope-expanded` | changes an unflagged prose field or changes entry/bullet cardinality |
| `gutted` | a changed field retains less than half of its original text |
| `not-cleaner` | unsupported count not strictly lower |
| `new-fabrication` | a changed field introduces a normalized number absent from that same original field |
...
- `pipeline-runner.ts` + `run-eval.ts`: mirror it, extend `--no-grounding-repair` to cover
  both documents, and report attempted, accepted, and provider-failed repairs separately for
  each document. Provider failures mirror production fallback and never resample the complete
  stochastic generation chain.
...
**Acceptance criteria**
- [x] Skips with no LLM call when the résumé has no unsupported numbers.
- [x] `isValidResumeEdit` rejection proven — a mangled `profileExperienceId` keeps the
      pre-repair payload.
- [x] Measured with Phase 1's tooling at `k≥3`, reported as claim rate + CI.
- [x] Attempted + accepted + failed counters distinguish clean skips, guard rejections,
  accepted repairs, and provider fallback.
...
| Repeated runs get expensive | Default `k=1`; use `k≥3` only for stochastic stability |
...
- Expanding the fixture set from 24. Fixtures are hand-authored and profession-balanced;
  `--repeat` reveals within-fixture variance but does not replace independent fixtures.
...
- **2026-08-12** — final review hardening: the résumé guard now rejects added/empty-ID
  entries, malformed prose fields, out-of-scope changes, locally gutted fields, and every newly
  introduced normalized number even when the profile-wide validator would otherwise ground it.
  Eval reports use `n/a` for zero checked values, expose repeat errors and metric-specific paired
  sample sizes, preserve historical single-run pairing, cluster repeated observations by fixture,
  and record provider failures separately without re-running the full stochastic generation chain.
- **2026-08-11** — `feat/grounding-hardening` Phase 2: added strict-JSON
  `fix-unsupported-numbers-resume.md`, `evaluateResumeGroundingRepair`, production + harness
  wiring after résumé style rewrite, explicit attempted/accepted/failed telemetry, and structured the
  legacy single-pipeline testing path so the same ID guard applies there. Focused tests prove a
  clean résumé makes zero LLM calls and a mangled `profileExperienceId` is rejected. A synthetic
  live probe repaired "Reduced response time by 40%" to "Reduced response time measurably"
  (1→0 unsupported). At `k=3` (72 outputs/arm across 24 fixture clusters), control measured
  5/180 unsupported claims = 2.78% (Wilson 95% CI [1.19, 6.34]); repair measured
  0/171 = 0.00% [0.00, 2.20], with
  6 accepted résumé repairs and 4 accepted cover repairs. These saved decision artifacts predate
  the final attempted/failed telemetry fields; the newer smoke artifacts exercise those fields.
  Per-repeat control rates were
  0.00%/4.92%/3.57%; all repair repeats were 0.00%. Clustered binary grounding improved 4
  fixtures and regressed 0 (exact McNemar p=0.1250); paired per-fixture mean grounding score
  improved +1.56 points (Student-t 95% CI [0.01, 3.10]), while judge overall and
  word count did not change significantly. Do not summarize this as a significant binary
  pass-rate result.
- **2026-08-11** — `feat/grounding-hardening` Phases 1b–1c: added `--repeat=N` pooling with
  `<tag>-rN` fixture attribution and per-repeat spread, plus `pnpm eval:compare` with exact
  McNemar and paired continuous 95% intervals. A live `--repeat=3 --limit=1` smoke produced
  one 3-fixture pooled artifact with 0/13 unsupported claims; repeat word counts were
  286/300/285, proving run-level variation remains visible. The full log contained no lane
  fallback, breaker, or provider warnings. Historical pre-repair → repair comparisons were
  not significant: gpt-4.1 had 3 improved / 2 regressed (McNemar p=1.000; grounding-score
  delta +0.46, 95% CI [-8.00, 8.92]); gpt-5.4-mini had 3 improved / 0 regressed (p=0.250;
  score delta +3.38, 95% CI [-1.54, 8.29]).
- **2026-08-11** — `feat/grounding-hardening` Phase 1a: added pooled unsupported-claim
  counts/rate and a Wilson 95% interval to `scripts/eval/aggregate.ts`, while retaining the
  historical fixture pass-rate. Token-free recomputation of the eight saved runs:
  `prose-gpt41` 3/65 = 4.62% [1.58, 12.71], `prose-gpt54mini` 1/50 = 2.00%
  [0.35, 10.50], `band-gpt41` 4/57 = 7.02% [2.76, 16.70], `band-gpt54mini` 5/63 =
  7.94% [3.44, 17.27], `nopad-gpt41` 5/58 = 8.62% [3.74, 18.64],
  `nopad-gpt54mini` 6/61 = 9.84% [4.59, 19.84], `repair-gpt41` 7/65 = 10.77%
  [5.32, 20.60], and `repair-gpt54mini` 1/50 = 2.00% [0.35, 10.50]. Intervals are
  percentages and overlap substantially; no model/prompt conclusion is warranted from these
  single runs.
