...
¹ *Only when `generateCoverLetter !== false`.* Step 1a and 1b run in **parallel** (`Promise.all`). Steps 2a/2b/2c run in **parallel** (`Promise.all`). Steps 2d, 2e, 4, 5, and 6 run sequentially after the parallel block. The grounding check (7) and style check (8) run on the finalized documents in the background (non-blocking). Resume is persisted as structured JSON for the editor; the cover letter is persisted as HTML for the PDF. Neither live path implements this chain any more — `createWithGeneration` (wizard) and `generateWithSinglePipeline` (regenerate) both call the shared `applications/headless/generate.ts`.
